...
1. 应用中心 → **手动安装** → 按设备架构选择安装包：
   - x86 设备：`fpk/lemon-music-1.0.13-x86.fpk`
   - ARM 设备：`fpk/lemon-music-1.0.13-arm.fpk`
2. 阅读「安装说明」「注意事项」
...
输出：`fpk/lemon-music-1.0.13-x86.fpk` 与 `fpk/lemon-music-1.0.13-arm.fpk`（不要提交到 git）。
