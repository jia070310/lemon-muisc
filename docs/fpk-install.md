...
1. 应用中心 → **手动安装** → 按设备架构选择安装包：
   - x86 设备：`fpk/lemon-music-1.0.18-x86.fpk`
   - ARM 设备：`fpk/lemon-music-1.0.18-arm.fpk`
2. 阅读「安装说明」「注意事项」
...
| 镜像标签 | `latest` 或指定版本如 `1.0.18`；安装脚本会按所选 tag 写入 compose 并拉取，**不会再被旧的 latest 配置覆盖** |
...
输出：`fpk/lemon-music-1.0.18-x86.fpk` 与 `fpk/lemon-music-1.0.18-arm.fpk`（不要提交到 git）。
