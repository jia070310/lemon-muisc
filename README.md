...
多平台关键词检索，结果列表支持单曲试听、加入试听队列、按音质下载；底栏播放器与试听列表常驻，可随时切换歌曲。

![搜索页：多平台检索、试听队列与底栏播放器](docs/screenshots/search.png)

### 发现 · 歌单

粘贴各平台歌单链接或浏览推荐歌单，查看完整歌曲列表，支持播放全部、批量加入队列与下载。

![发现页：歌单推荐与多平台歌单解析](docs/screenshots/discover.png)
...
- 底栏 **播放器**：播放 / 暂停、上一首 / 下一首、进度条、音量
- **试听列表**：可拖拽排序（持久化到本地），支持列表循环、单曲循环、随机播放
- 封面样式可选：圆形唱片（旋转）或圆角卡片
- 深色 / 浅色主题切换
- 对外链音频走服务端代理，减少浏览器 CORS / Referer 导致的无法播放
...
- 安装向导支持镜像拉取进度、国内加速地址（如 `ghcr.1ms.run`）
...
生成 `fpk/lemon-music.fpk`，在飞牛 **应用中心 → 手动安装**。安装向导、镜像加速与目录权限见 [docs/fpk-install.md](docs/fpk-install.md)。
