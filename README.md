...
- **多用户数据隔离**：歌单 / 收藏 / 最近播放按账号分开；本地缓存随登录用户切换，避免串数据
- **下载路径**：共享默认目录 + 可选个人目录（默认 `{共享路径}/{用户名}/`）
- **音源按用户启用**：普通用户可自行启用 / 停用已导入音源；导入与删除仍仅管理员
- **澜音 / CeruMusic 原生音源**：除落雪脚本外，支持 Ceru 风格 `module.exports` 音源
- **试听片段拦截**：播放提示疑似预览；下载前探测时长，短片段跳过并换源重试
- **网络歌单更新**：新增曲目插入到歌单最前
...
| **音源管理** | 导入落雪 / 澜音脚本；各用户可独立启用多个音源 |
| **风格样式** | 主题、配色、封面样式、音频可视化、音乐库歌曲列数 |
| **下载设置** | 路径、文件名、并发、分组、同名文件处理（询问/跳过/覆盖） |
| **账号 / 邮件** | 多用户管理（管理员）、个人歌单数据隔离、SMTP 邮箱验证与找回密码 |
...
生成 `fpk/lemon-music-1.2.7-x86.fpk` 与 `fpk/lemon-music-1.2.7-arm.fpk`。说明见 [docs/fpk-install.md](docs/fpk-install.md)。
