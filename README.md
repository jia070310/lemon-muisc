...
- **后台播放**：锁屏 / 切后台时尽量继续用原生音频通道播放（频谱分析不劫持输出）
- 封面样式：圆形唱片（旋转）或圆角卡片
...
- **卸载向导**：单页单选「保留应用数据 / 删除应用数据」，不会删除用户自己的音乐文件
...
> FPK 需按设备架构选择：`lemon-music-1.0.15-x86.fpk` 或 `lemon-music-1.0.15-arm.fpk`。
...
生成 `fpk/lemon-music-1.0.15-x86.fpk` 与 `fpk/lemon-music-1.0.15-arm.fpk`，在飞牛 **应用中心 → 手动安装** 选择与你设备架构对应的安装包（不要点卡片「更新」）。
