...
> FPK 需按设备架构选择：`lemon-music-1.0.16-x86.fpk` 或 `lemon-music-1.0.16-arm.fpk`。
...
生成 `fpk/lemon-music-1.0.16-x86.fpk` 与 `fpk/lemon-music-1.0.16-arm.fpk`，在飞牛 **应用中心 → 手动安装** 选择与你设备架构对应的安装包（不要点卡片「更新」）。
